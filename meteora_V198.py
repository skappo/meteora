...
DEFAULTS = {
    "capture": {
        ...
        "max_attempts": 3,
        "max_brightness": 240,
        "moon_normal_penalty": 8,      # 15 - Penalty when moon is out of FOV
        "moon_max_penalty": 40,        # Penalty when moon is in FOV
        "ae_step_limit_up": 4.0,       # Max multiplier for one AE check
        "ae_step_limit_down": 0.25,    # Min multiplier (0.25 = 4x darker)
    },
    "detection": {
        "min_area": 50,
        "pre_event_frames": 8,
        ...
        "min_threshold": 10,
        "max_threshold": 100,
        "target_adaptation_time": 20.0  # <--- Background model absorption speed
    },
    "timelapse": {
        "stack_N": 20,
        "stack_align": True,
        ...
        "cloudy_interval_sec": 60,       # Cloudy throttle interval
        "alignment_threshold_floor": 3,  # Minimum pixel value to consider for star mask
        "stretch_dark_floor": 0.008,     # Midpoint for black-sky transition
        "stretch_transition_width": 0.006, # Width of the lux-aware blending zone
        "stretch_noise_percentile": 2.0,    # Percentile to sample for noise floor (2.0)
        "stretch_noise_cap": 0.02,         # Max noise floor addition (0.02)
        "stretch_weight_threshold": 0.4,   # Tipping point for chroma suppression (0.4)
        "stretch_ratio_low_lux": 3.0,      # Max color boost in dark scenes (3.0)
        "stretch_ratio_high_lux": 8.0,     # Max color boost in clear scenes (8.0)
        "stretch_astro_threshold": 0.012,  # Start of "True Astro" regime (0.012)
        "stretch_astro_width": 0.01        # Width of astro blend zone (0.01)
    },
    "timelapse_video": {
        "enabled": True,
        "video_fps": 10,
...
class CaptureConfig(BaseModel):
    ...
    moon_normal_penalty: conint(ge=0, le=100) = DEFAULTS["capture"]["moon_normal_penalty"]
    moon_max_penalty: conint(ge=0, le=100) = DEFAULTS["capture"]["moon_max_penalty"]
    ae_step_limit_up: confloat(gt=1.0, le=10.0) = DEFAULTS["capture"]["ae_step_limit_up"]
    ae_step_limit_down: confloat(gt=0.0, lt=1.0) = DEFAULTS["capture"]["ae_step_limit_down"]
...
class DetectionConfig(BaseModel):
    ...
    target_adaptation_time: confloat(gt=0.0) = DEFAULTS["detection"]["target_adaptation_time"]
...
class TimelapseConfig(BaseModel):
    ...
    cloudy_interval_sec: conint(ge=0) = DEFAULTS["timelapse"]["cloudy_interval_sec"]
    alignment_threshold_floor: conint(ge=1, le=50) = DEFAULTS["timelapse"]["alignment_threshold_floor"]
    stretch_dark_floor: confloat(gt=0.0) = DEFAULTS["timelapse"]["stretch_dark_floor"]
    stretch_transition_width: confloat(gt=0.0) = DEFAULTS["timelapse"]["stretch_transition_width"]
    stretch_noise_percentile: confloat(ge=0.1, le=10.0) = DEFAULTS["timelapse"]["stretch_noise_percentile"]
    stretch_noise_cap: confloat(ge=0.0, le=0.1) = DEFAULTS["timelapse"]["stretch_noise_cap"]
    stretch_weight_threshold: confloat(ge=0.1, le=0.9) = DEFAULTS["timelapse"]["stretch_weight_threshold"]
    stretch_ratio_low_lux: confloat(ge=1.0, le=5.0) = DEFAULTS["timelapse"]["stretch_ratio_low_lux"]
    stretch_ratio_high_lux: confloat(ge=5.0, le=20.0) = DEFAULTS["timelapse"]["stretch_ratio_high_lux"]
    stretch_astro_threshold: confloat(gt=0.0) = DEFAULTS["timelapse"]["stretch_astro_threshold"]
    stretch_astro_width: confloat(gt=0.0) = DEFAULTS["timelapse"]["stretch_astro_width"]
    @model_validator(mode='after')
    def validate_thresholds(self):
        ...
        dark_thresh = self.auto_sum_darkness_threshold
        bright_thresh = self.auto_stack_brightness_threshold
...
class MonitorConfig(BaseModel):
    ...
    ram_limit_percent: confloat(ge=50.0, le=95.0) = DEFAULTS["monitor"]["ram_limit_percent"]
    ram_recovery_percent: confloat(ge=40.0, le=90.0) = DEFAULTS["monitor"]["ram_recovery_percent"]
...
class DaylightConfig(BaseModel):
    ...
    min_star_circularity: confloat(ge=0.1, le=1.0) = DEFAULTS["daylight"]["min_star_circularity"]
...
class GeneralConfig(BaseModel):
    ...
    jpg_quality: conint(gt=0,le=100) = DEFAULTS["general"]["jpg_quality"]
    jpg_quality_zmq: conint(gt=0,le=100) = DEFAULTS["general"]["jpg_quality_zmq"]
    ...
    shutdown_time: Optional[constr(pattern=r'^(?:[01]\d|2[0-3]):[0-5]\d$')] = DEFAULTS["general"]["shutdown_time"]
    start_time: Optional[constr(pattern=r'^(?:[01]\d|2[0-3]):[0-5]\d$')] = DEFAULTS["general"]["start_time"]
    end_time: Optional[constr(pattern=r'^(?:[01]\d|2[0-3]):[0-5]\d$')] = DEFAULTS["general"]["end_time"]
...
class MainConfig(BaseModel):
    ...
    def check_stack_n_memory_usage(self):
        ...
        capture_cfg = self.capture
        timelapse_cfg = self.timelapse
        
        # 1. Estimate memory per frame (width * height * 3 bytes/pixel)
        bytes_per_frame = capture_cfg.width * capture_cfg.height * 6
        ...
        safe_max_stack_n = max(5, safe_max_stack_n) # Failsafe minimum
        ...
        return self # Must return the value for the next validator
    # --------------------------- Logging ---------------------------
...
class CameraCapture:
    def __init__(self, cfg):
        ...
        controls = {
            "ExposureTime": cfg.get("exposure_us"),
            "AnalogueGain": cfg.get("gain"),
...
class Pipeline:
    ...
    def __init__(self, cfg, config_path=None, force_rotation=False):
        ...
        self.rejected_out_dir = os.path.join(base_path, "output/rejected")
        self.debug_out_dir = os.path.join(base_path, "output/debug")
        ...
        self.event_log_q = queue.Queue(maxsize=1000)                        # Queue for the event logger
        self.health_q = queue.Queue(maxsize=1000)                           # Queue for health statistics
        self.calibration_q = queue.Queue(maxsize=100)                       # Queue for the calibration image
        self.sftp_dispatch_q = queue.Queue(maxsize=1000)                    # Queue for SFTP dispatcher
        ...
        self.memory_critical = threading.Event()                            # --- RAM Protection Flag ---
...
#        self.last_queue_status = ("ok", "Starting...") # Holds (css_class, message)
    ...
    def _initial_cleanup(self):
        ...
        cleanup_count = 0
        if os.path.isdir(self.events_out_dir):
            try:
                with os.scandir(self.events_out_dir) as it:
                    for entry in it:
        ...
        # --- Clean up legacy nested debug directories to free disk space ---
        for legacy_dir in ["debug_dumps", "debug_detection"]:
            legacy_path = os.path.join(self.general_log_dir, legacy_dir)
            if os.path.isdir(legacy_path):
                try:
                    shutil.rmtree(legacy_path)
                    logging.info("Initial cleanup removed legacy debug directory: %s", legacy_dir)
                except Exception: pass
    # -----------------
    ...
    def _load_chronicle(self):
        ...
        return []
    # -----------------
    def _update_chronicle(self, hours, sky_avg, clarity, sky_history):
        record = {
            "date": self.session_stats["start_time"].strftime("%Y-%m-%d"),
            "hours_watched": round(hours, 2),
            "events_accepted": self.session_stats["accepted_events"],
            "clarity_pct": round(clarity, 1),
            "sky_score_avg": round(sky_avg, 3),
            "peak_temp_c": round(self.session_stats["peak_cpu_temp"], 1),
            "max_moon_impact": round(self.session_stats["max_moon_impact"], 3),
            "data_written_mb": round(self.session_stats["data_written_mb"], 1),
    ...
    def _get_chronicle_context(self):
        ...
        tonight_events  = self.session_stats["accepted_events"]
        tonight_clarity = (self.session_stats["clear_sky_minutes"] / max(1, self.session_stats["clear_sky_minutes"] + self.session_stats["cloudy_sky_minutes"]) * 100)
        tonight_score   = float(np.mean(self.session_stats["avg_sky_score"])) if self.session_stats["avg_sky_score"] else 0.0
    ...
    def _apply_resolution_aware_tuning(self):
        ...
        self.blur_size = profile["blur"]
        self.morph_size = profile["morph"]
        ...
        while self.running.is_set():
            ...
            if self.camera_fatal_error.is_set():    # the camera reach max_camera_failures
                ...
                if not self.producer_thread_active():
                    ...
                    logging.critical("Producer thread failed to restart. (%d/%d)", self.consecutive_restart_failures, max_restart_attempts)
    ...
    def generate_morning_journal(self):
        ...
        mean_lux = np.mean(self.session_stats["avg_lux"]) if self.session_stats["avg_lux"] else 0.0
              
        clarity = (self.session_stats["clear_sky_minutes"] / max(1, self.session_stats["clear_sky_minutes"] + self.session_stats["cloudy_sky_minutes"]) * 100)
        sky_avg = np.mean(self.session_stats["avg_sky_score"]) if self.session_stats["avg_sky_score"] else 0.0

        fp  = self._generate_atmosphere_fingerprint(self.session_stats.get("sky_history", []))
        ctx = self._get_chronicle_context()
        self._update_chronicle(hours, sky_avg, clarity, self.session_stats.get("sky_history", []))

        eff = 100.0
        if self.session_stats["total_events"] > 0:
            eff = 100 - (self.session_stats["rejected_contours"] / self.session_stats["total_events"] * 100)        
        
        lines = [f"THE WATCHMAN'S JOURNAL | Night of {self.session_stats['start_time'].strftime('%B %d, %Y')} Session Ending"]
        lines.append("-" * 60)
        ...
        if self.session_stats["max_moon_impact"] > 0.1:
            lines.append(f"• LUNAR INFLUENCE: The Moon was in FOV for {self.session_stats['moon_visible_minutes']:.2f}m (Max Impact: {int(self.session_stats['max_moon_impact']*100)}%).")
        
        lines.append(f"• EFFICIENCY: Analyzed {self.session_stats["total_events"]:,} potential triggers; ignored {self.session_stats["rejected_contours"]:,} noise artifacts ({eff:.1f}% accuracy).")
        lines.append(f"• DETECTIONS: Isolated {self.session_stats["accepted_events"]} candidate celestial events.")
        lines.append(f"• STORAGE: Recorded {self.session_stats["data_written_mb"]:.1f} MB of new astronomical data to disk.")
        lines.append(f"• VITALS: Peak operating temperature: {self.session_stats['peak_cpu_temp']:.1f}°C.")
        ...
        try:
            ...
            if self.session_stats.get("sky_history") and len(self.session_stats["sky_history"]) > 1:
                fig, ax1 = plt.subplots(figsize=(10, 5), facecolor='#1e1e1e')
                ax1.set_facecolor('#1e1e1e')
                
                # Define Labels and Line Colors
                C_SKY = '#61afef'
                C_MOON = '#e5c07b'
                C_STARS = '#98c379'
                C_NOISE = '#c678dd'
                ...
                # --- PRIMARY AXIS (Left): Sky Score & Moon Penalty ---
                t_sky = [datetime.fromtimestamp(h["t"], tz=timezone.utc) for h in self.session_stats["sky_history"]]
 
                # 1. Sky Score (Cyan)
                scores = [h["score"] for h in self.session_stats["sky_history"]]
                line1, = ax1.plot(t_sky, scores, color=C_SKY, marker='o', markersize=3, linewidth=2, label='Sky Score', zorder=5)                
                ...
                # 2. Moon Penalty (Orange/Amber)
                if self.session_stats.get("moon_penalty_history"):
                    t_moon = [datetime.fromtimestamp(h["t"], tz=timezone.utc) for h in self.session_stats["moon_penalty_history"]]
                    penalties = [h["val"] for h in self.session_stats["moon_penalty_history"]]
                    line2, = ax1.plot(t_moon, penalties, color=C_MOON, linestyle='--', alpha=0.6, label='Moon Penalty', zorder=4)
                
                # ax1.set_ylabel('Score / Penalty', color='#61afef', fontweight='bold')
                ax1.tick_params(axis='y', colors=C_SKY)
                ax1.grid(color='#333333', linestyle=':', alpha=0.5)

                # --- MULTI-COLOR LEFT LABEL ---
                # Place label "Score / Penalty" the same color of own graph
                ax1.annotate('Score', xy=(-0.07, 0.65), xycoords='axes fraction', rotation=90, ha='center', va='center', color=C_SKY, fontweight='bold', fontsize=11)
                ax1.annotate('/', xy=(-0.07, 0.5), xycoords='axes fraction', rotation=90, ha='center', va='center', color=C_DIVIDER, fontweight='bold', fontsize=11)
                ax1.annotate('Penalty', xy=(-0.07, 0.32), xycoords='axes fraction', rotation=90, ha='center', va='center', color=C_MOON, fontweight='bold', fontsize=11)

                # --- SECONDARY AXIS (Right): Stars & StdDev ---
                ax2 = ax1.twinx()
                
                # 3. Star Count (Green)
                if self.session_stats.get("star_count_history"):
                    t_stars = [datetime.fromtimestamp(h["t"], tz=timezone.utc) for h in self.session_stats["star_count_history"]]
                    stars = [h["val"] for h in self.session_stats["star_count_history"]]
                    line3, = ax2.plot(t_stars, stars, color=C_STARS, marker='x', markersize=4, linestyle='-', alpha=0.8, label='Stars', zorder=3)
                
                # 4. StdDev (Purple/Magenta)
                if self.session_stats.get("stddev_history"):
                    t_std = [datetime.fromtimestamp(h["t"], tz=timezone.utc) for h in self.session_stats["stddev_history"]]
                    stddevs = [h["val"] for h in self.session_stats["stddev_history"]]
                    line4, = ax2.plot(t_std, stddevs, color=C_NOISE, linestyle=':', alpha=0.7, label='Noise (StdDev)', zorder=2)
                
                # ax2.set_ylabel('Count / Noise', color='#98c379', fontweight='bold')
                ax2.tick_params(axis='y', colors='#98c379')

                # --- MULTI-COLOR RIGHT LABEL ---
                # Place label "Count / Noise" the same color of own graph
                ax2.annotate('Count', xy=(1.07, 0.65), xycoords='axes fraction', rotation=270, ha='center', va='center', color=C_STARS, fontweight='bold', fontsize=11)
                ax2.annotate('/', xy=(1.07, 0.5), xycoords='axes fraction', rotation=270, ha='center', va='center', color=C_DIVIDER, fontweight='bold', fontsize=11)
                ax2.annotate('Noise', xy=(1.07, 0.35), xycoords='axes fraction', rotation=270, ha='center', va='center', color=C_NOISE, fontweight='bold', fontsize=11)

                # --- TERTIARY AXIS (Overlay): Bias Mode ---
                if self.session_stats.get("bias_history"):
                    t_bias = [datetime.fromtimestamp(h["t"], tz=timezone.utc) for h in self.session_stats["bias_history"]]
                    bias_vals = [1 if h["bias"] else 0 for h in self.session_stats["bias_history"]]
                    ax3 = ax1.twinx()
                    # Offset the axis so it doesn't overlap labels
                    ax3.spines.right.set_position(("axes", 1.15)) 
                    ax3.fill_between(t_bias, 0, bias_vals, color='#e06c75', alpha=0.2, step='post')
                    ax3.set_yticks([]) # Hide Y ticks for the background fill
                    ax3.set_ylim(0, 1)

                # Legends and Formatting
                ax1.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
                plt.title(f"Celestial Metrics & Atmospheric Stability", color='#ffffff', pad=25, fontsize=14)
                
                # Combine legends from both axes
                plot_lines  = [line1]
                if self.session_stats.get("moon_penalty_history"): plot_lines.append(line2)
                if self.session_stats.get("star_count_history"): plot_lines.append(line3)
                if self.session_stats.get("stddev_history"): plot_lines.append(line4)
                
                labels = [l.get_label() for l in plot_lines]
                ax1.legend(plot_lines, labels, loc='upper left', fontsize='small', frameon=True, facecolor='#1e1e1e', edgecolor='#444444')

                fig.tight_layout()
                plot_path = os.path.join(self.general_log_dir, f"temp_plot_{int(time.time())}.png")
                plt.savefig(plot_path, dpi=200, facecolor=fig.get_facecolor())
                plt.close(fig)
                
        except Exception as e:
            pdf_supported = False
            logging.error(f"Failed to generate complex journal graph: {e}")

        # --- 2. DOCUMENT GENERATION (Elegant PDF) ---
        rpt_name_base = self.session_stats["start_time"].strftime("journal_%Y%m%d")
        ...
        if pdf_supported:
            try:
                ...
                add_section("Mission Parameters", {
                    ...
                    "Peak Temperature:": f"{self.session_stats['peak_cpu_temp']:.1f} °C",
                    "Data Recorded:": f"{self.session_stats['data_written_mb']:.1f} MB",
                })
                
                add_section("Detection Analytics", {
                    "Sky Clarity:": f"{clarity:.1f}% (Avg Score: {sky_avg:.2f})",
                    "Celestial Events:": f"{self.session_stats["accepted_events"]} isolated",
                    "Detector Efficiency:": f"{eff:.1f}% ({self.session_stats["rejected_contours"]:,} noise artifacts rejected)",
                })
                ...
                add_section("Atmospheric Telemetry", {
                    ...
                    "Lunar Impact:": f"In FOV for {self.session_stats['moon_visible_minutes']:.1f}m (Max Flare: {int(self.session_stats['max_moon_impact']*100)}%)" if self.session_stats["max_moon_impact"] > 0.1 else "None",
                })
    ...
    def stop_producer_thread(self):
        ...
        # Signal SkyMonitor to abort immediately
        self.capture_running.clear() 
        
        # Give the SkyMonitor a moment to see the 'clear' signal and exit its lock
        time.sleep(0.5) 
    ...
    def capture_loop(self): 
        ...
        while self.running.is_set() and self.capture_running.is_set():
            ...
            try:
                self.acq_q.put_nowait((ts, frame))
            except queue.Full:
    ...
    def perform_auto_exposure(self):
        ...
        base_target = cfg.get("target_sky_brightness")
        target = base_target + (cfg.get("moon_normal_penalty") * moon_impact) if in_fov else base_target
        
        base_max = cfg.get("max_brightness", 240)
        effective_max = min(255, base_max + (cfg.get("moon_max_penalty") * moon_impact if in_fov else cfg.get("moon_normal_penalty") * moon_impact))
        ...
        for attempt in range(max_attempts):
            if not self.running.is_set() or not self.capture_running.is_set():
                logging.warning("Auto-exposure aborted: Shutdown initiated.")
                return 
    ...
    def calculate_lux(self, brightness, exposure_us, gain):
        ...
        # If for calculate calibration_k the image used is pitch-black the value is too low (0.002)
        # if that the calculate_lux jump to very high value (800) and trick the pipeline into thinking the sun just came up
        # so 0.5 is a floor level for the math
    ...
    def _set_camera_preset(self, mode="NIGHT", **overrides):
        ...
        presets = {
            "NIGHT": {
                ...
                "ColourGains":[self.cfg["capture"].get("red_gain"), self.cfg["capture"].get("blue_gain")]
            },
            "DAY": {
                "AeEnable": True, 
                "AwbEnable": True,
    ...
    def _get_dynamic_accumulate_alpha(self):
        ...
        alpha = exp_seconds / self.cfg["detection"].get("target_adaptation_time")
    ...
    def detection_loop(self):
        ...
        while self.running.is_set():
            ...
            new_buffer_size = self.cfg["detection"].get("pre_event_frames")
            cooldown_frames = self.cfg["detection"].get("event_cooldown_frames")
            ...
            if self.is_calibrating.is_set():
                try:
                    self.calibration_q.put_nowait((ts, frame))
                except queue.Full:
            ...
            if effective_threshold is None or not isinstance(effective_threshold, (int, float)):
...
#            gray = cv2.GaussianBlur(gray, (blur_size, blur_size), 0)
            # Use the pre-tuned blur size from the profile
            ...
            # Use the pre-tuned morphology size to join gaps in the meteor trail
            morph_kernel = np.ones((self.morph_size, self.morph_size), np.uint8)
            th = cv2.morphologyEx(th, cv2.MORPH_CLOSE, morph_kernel)
            ...
            for c in cnts:
                ...
                if area < self.min_area:
                    ...
                    rejected_cnts_list.append(c)
                else:
                    accepted_cnts += 1
            
            detected = (accepted_cnts > 0)

            # --- REJECTED EVENT DIAGNOSTIC SNAPSHOT ---
            if not detected:
                now = time.time()
                # Rate limit: Max 1 diagnostic image every 10 seconds to prevent disk spam

                if now - self.last_reject_save_time > 10.0:
                    self.last_reject_save_time = now
                    
                    # Convert the 16-bit array to standard 8-bit for a lightweight JPEG
                    rej_img = (frame >> 8).astype(np.uint8)
                    
                    # Draw RED circles around what OpenCV saw so you know WHY it was rejected
                    if len(rejected_cnts_list) > 0:
                        # cv2.drawContours(rej_img, rejected_cnts_list, -1, (0, 0, 255), 2)
                        cv2.drawContours(rej_img, rejected_cnts_list, -1, (0, 0, 255), 1)
                        
                    rej_name = f"rejected_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.jpg"
                    rej_path = os.path.join(self.rejected_out_dir, rej_name)
                    
                    logging.info(f"Diagnostics: False positive rejected. Saved snapshot to {rej_name}")
                    self._safe_save_image(rej_path, rej_img,[int(cv2.IMWRITE_JPEG_QUALITY), self.cfg["general"]["jpg_quality"]])
                ...
                if debug_level >= 3:
                    ...
                    if debug_level == 2 and self.event_in_progress.is_set():
                        ...
                        if now - last_debug_save > debug_save_interval:
                            fname = os.path.join(self.debug_out_dir, f"det_{datetime.now().strftime('%H%M%S_%f')}.jpg")
            ...
            recent_frames.append((ts, frame, effective_threshold))
...
#                        self.session_stats["rejected_contours"] += rejection_stats["total"]
                    ...
                    current_event_video_frames.append((ts, frame, effective_threshold))
                    current_event_stack_frames.append((ts, frame, effective_threshold))
            ...
            # --- EVALUATE EVENT END CONDITIONS ---
            is_cooldown_end = (event_cooldown_counter > cooldown_frames)
            is_timeout_end = (current_event_frame_count >= max_event_frames)
            is_memory_kill = self.memory_critical.is_set()
            
            # If ANY of these 3 conditions are true, shut down the event!
            # if self.event_in_progress.is_set() and (is_cooldown_end or is_timeout_end or is_memory_kill):
#            logging.info(f"ecc {event_cooldown_counter} su {cooldown_frames} - cef {current_event_frame_count} su {max_event_frames}")
 ...
 #               self.background = None
    ...
    def analyze_celestial_context(self) -> Tuple[SkyConditionStatus, dict]:
        ...
        with self.camera_lock:        
            try:     
                ...
                if frame is not None:
                    ...
                    if getattr(self, "use_ram_for_check", False) and getattr(self, "last_raw_frame", None) is not None:
                        ...
                        if not self.capture_running.is_set():
                            return SkyConditionStatus.ERROR, {"message": "Shutdown initiated"}
                ...
                self.session_stats["star_count_history"].append({"t": now_ts, "val": int(results["stars"])})
                self.session_stats["stddev_history"].append({"t": now_ts, "val": round(float(results["stddev"]), 2)})
                self.session_stats["moon_penalty_history"].append({"t": now_ts, "val": round(moon_penalty, 2)})
                ...
                if is_clear:
                    ...
                    if self.timelapse_interval_sec <= self.cfg["timelapse"].get("cloudy_interval_sec"):
                        logging.info(f"The conditions are cloudy, so timelapse interval time is increase to {self.cfg["timelapse"].get("cloudy_interval_sec")} sec.")
                        self.base_timelapse_interval = self.cfg["timelapse"].get("cloudy_interval_sec")
                        self.timelapse_interval_sec = self.base_timelapse_interval
    ...
    def analyze_sky_conditions(self, img):
        ...
        base_k = self.star_mask_radius * self.top_hat_factor    # 3
        ...
        for c in cnts:
            ...
            if min_star_area <= area <= max_star_area:
                ...
                if circularity > self.cfg["daylight"].get("min_star_circularity"): # Stars are usually > 0.7, 0.6 is safe.
                    star_count += 1
    ...
    # -----------------
    def compute_moon_penalty(self, moon_impact, moon_alt, moon_in_fov):
        """
        ...
        # base scale
        scale = self.cfg["capture"].get("moon_max_penalty") if moon_in_fov else self.cfg["capture"].get("moon_normal_penalty")
    ...
    def wait_for_safe_window(self):
        ...
        return True
    # -----------------
    def _compute_roi_stats(self, frame, blur=False, star_candidates=None, radius=5, min_features=3, save_debug=None, debug_out_dir=False):
        """
        ...
        if star_candidates:
            ...
            roi_meta.update({"anchor_points": anchors_in_center, "mask": mask, "valid": (cv2.countNonZero(mask) >= self.min_mask_px and anchors_in_center >= min_features)})
...
#        top_stars = 0
        ...
        stack_dump_prefix = None
        if self.cfg["general"].get("debug_level") == 2:   
            timestamp = datetime.now().strftime("%H%M%S")
            stack_type = "event" if is_event_stack else "timelapse"
            stack_dump_prefix = f"stack_{stack_type}_{timestamp}"
        ...
        try:
            ...
            def create_star_mask(gray_img, percentile=97.0, min_area=2, max_area=400, radius=None, max_stars=200, min_features=3, save_debug=debug_mode, bias_mode=False, debug_out_dir=self.debug_out_dir):
                try:
                    if radius is None:
                    ...
                    # Final threshold
                    tval = max(self.cfg["timelapse"].get("alignment_threshold_floor"), t_sigma, t_percentile)
...
# star integrity over maximum contrast.
            ...
            def astro_stretch(img,bias_mode=False):
                ...
                low_lux_weight = np.clip((self.cfg["timelapse"].get("stretch_dark_floor") - median_lum) / self.cfg["timelapse"].get("stretch_transition_width"), 0, 1)              
                ...
                noise_floor = min(np.percentile(lum, self.cfg["timelapse"].get("stretch_noise_percentile")), self.cfg["timelapse"].get("stretch_noise_cap"))
                ...
                # ── Blend stretches
                median_lum = np.median(lum)
                w_astro = np.clip((median_lum - self.cfg["timelapse"].get("stretch_astro_threshold")) / self.cfg["timelapse"].get("stretch_astro_width"), 0, 1)
                w_hybrid = np.clip(1.0 - low_lux_weight - w_astro, 0, 1)

                lum_stretched = (low_lux_weight * lum_low + w_hybrid * lum_hybrid + w_astro * lum_astro)
                ...
                max_ratio = self.cfg["timelapse"].get("stretch_ratio_low_lux") if low_lux_weight > self.cfg["timelapse"].get("stretch_weight_threshold") else self.cfg["timelapse"].get("stretch_ratio_high_lux")
...
#                logging.info(f"Bias mode: {'ON' if bias_mode else 'OFF'} (anchors={anchors_used}, std={star_meta.get('stddev',0):.2f})")
                    ...
                    for i in range(len(frames)):
                        ...
                        if perform_alignment:
                            try:
                                ...
                                gauss_size = (self.ecc_gauss_size + 2) if bias_mode else self.ecc_gauss_size
                                # CAPTURE METRICS
                                ...
                                MAX_WARP_PIXELS = min(w, h) * self.max_warp_factor    # 0.05   # old 0.10 - reject if shift > 10% of frame dimension
                                ...
                                if self.cfg["general"].get("debug_level") == 3:
                                    ...
                                    if mosaic is not None:
                                        ...
                                        if stack_dump_dir:
                                            ...
                                            self._safe_save_image(os.path.join(self.debug_out_dir, f"{stack_dump_prefix}_align_{i:03d}_cc{int(cc*100)}.jpg"), mosaic)
    ...
    def get_moon_state(self, calc_date=None):
        ...
        try:
            ...
            # Camera Orientation
            c_alt = self.cfg["general"].get("camera_altitude") 
            c_az = self.cfg["general"].get("camera_azimuth")
            h_fov = self.cfg["general"].get("lens_hfov")
            v_fov = self.cfg["general"].get("lens_vfov")
    ...
    def event_writer_loop(self):
        ...
        while True:
            ...
            bitrate = str(self.cfg["events"]["ffmpeg_bitrate"])
            ...
            try:
                ...
                if process.returncode != 0:
                    ...
                    if self.sftp_uploader and self.running.is_set():
                        try:
                            self.sftp_dispatch_q.put_nowait(video_path)
                            self.sftp_dispatch_q.put_nowait(os.path.splitext(video_path)[0] + ".json")
                        except queue.Full:
                            logging.error(f"SFTP Dispatch queue full. {os.path.basename(video_path)} will stay on disk but won't be uploaded this session.")
                    ...
                    if buffer:
                        ...
                        if stacked_image is not None and final_method is not None:
                            ...
                            if self.sftp_uploader and self.running.is_set():
                                ...
                                self.sftp_dispatch_q.put_nowait(full_path)
                                self.sftp_dispatch_q.put_nowait(os.path.splitext(full_path)[0] + ".json")
        ...
        try:
            ...
            for path in image_files:
                ...
                try:
                    ...
                    historical_target = base_target + (self.cfg["capture"].get("moon_normal_penalty") * moon_impact) if in_fov else base_target
    ...
    def system_monitor_loop(self):
        ...
        while self.running.is_set():
            ...
            if mem.percent >= self.cfg["monitor"].get("ram_limit_percent"):
                if not self.memory_critical.is_set():
            ...
            elif mem.percent <= self.cfg["monitor"].get("ram_recovery_percent"):
                if self.memory_critical.is_set():
    ...
    def dashboard_loop(self):
        ...
        # --- PARAMETER ---
        EDITABLE_PARAMS = {
            "capture": ["exposure_us", "gain", "red_gain", "blue_gain", "moon_normal_penalty", "moon_max_penalty"],
            "detection":["target_adaptation_time"],
            "timelapse": ["stack_N", "cloudy_interval_sec"],
        }
        ...
        def system_time():
            ...
            if request.method == 'POST':
                try:
                    ...
                    # Validate the format using datetime.strptime
                    # This raises a ValueError if the string is malformed or not a real date.
                    try:
                        # Format with seconds: "YYYY-MM-DD HH:MM:SS"
                        parsed_dt = datetime.strptime(f"{new_date.strip()} {new_time.strip()}", "%Y-%m-%d %H:%M:%S")
                    except ValueError:
                        # Fallback for browsers that omit seconds (e.g. "YYYY-MM-DD HH:MM")
                        parsed_dt = datetime.strptime(f"{new_date.strip()} {new_time.strip()}", "%Y-%m-%d %H:%M")

                    # Rebuild the string directly from the validated Python object 
            ...
            if self.within_schedule():
                ...
                with self.camera_lock:
                    try:
                        ...
                        logging.info("Hardware Start: Success.")
                        test_frame = self.camera.read()
                        if test_frame is None:
                            raise RuntimeError("Camera started but returned empty frame during validation.")
                        logging.info("Hardware Validation: Success (Sensor responsive).")
                    except Exception as e:
                        # HARDWARE FAILURE PATH
                        error_msg = f"Hardware Failure: Could not restart sensor. Reason: {e}"
                        logging.critical(error_msg)
                        
                        # Log to the health system so it appears on the dashboard
                        self.log_health_event("CRITICAL", "HARDWARE_RESUME_FAIL", str(e))
                        
                        # Force Maintenance Mode back ON to prevent thread loops from spinning
                        self.in_maintenance_mode.set()
                        self.camera_fatal_error.set() # Triggers the scheduler logic to attempt a full restart later
                        
                        # Tell the user exactly what happened
                        return redirect(url_for('dashboard', token=DASH_TOKEN, resume_status='fail', error=str(e)))                        
                self.capture_stable.clear() # Force SkyMonitor to run a fresh calibration
                self.request_sky_analysis_trigger.set()
                self.start_producer_thread()
...
#            import html
            ...
            elif resume_status == 'fail':
                # Red banner for hardware failure
                feedback_html = f'<div class="banner banner-danger" style="background-color: #8b0000; color: white;">' \
                                f'<strong>CRITICAL HARDWARE ERROR:</strong> {resume_error}</div>'
...
#                            settings_html_rows += f'<tr><td>{param}</td><td><input type="text" name="{section}_{param}" value="{value}" size="10"> {note}</td></tr>'
            ...
            actual_duration = datetime.now(timezone.utc) - self.session_stats["start_time"]
            dur_total_min = max(1, int(actual_duration.total_seconds() / 60))
            
            # Use categories for weather %
            measured_min = (self.session_stats["clear_sky_minutes"] + self.session_stats["cloudy_sky_minutes"]) or 1
            cloud_percent = (self.session_stats["cloudy_sky_minutes"] / measured_min) * 100
            
            avg_score = sum(self.session_stats["avg_sky_score"]) / len(self.session_stats["avg_sky_score"]) if self.session_stats["avg_sky_score"] else 0
            ...
            noise_ratio = self.session_stats["rejected_contours"] / (self.session_stats["total_events"] or 1)
            ...
            <div class="card" style="border: 1px solid #c678dd; background: linear-gradient(145deg, #2b2b2b, #352b3b);">
                ...
                <table style="font-size: 1.1em;">
                    ...
                    <tr><td>Detection Efficiency</td><td>{self.session_stats['total_events']} Events / {self.session_stats['rejected_contours']} Noise Blobs</td></tr>
    ...
    def publish_debug(self, msg_type, payload, image=None):
        ...
        try:
            ...
            if image is not None:
                ret, jpg = cv2.imencode(".jpg", image, [cv2.IMWRITE_JPEG_QUALITY, self.cfg["general"]["jpg_quality_zmq"]])
                if ret:
...
if __name__ == '__main__':
    ...
    def capture_calib_stack(pipeline, count, desc, out_name):
        ...
        # Give the sensor time to stabilize after settings change
        time.sleep(2) 

        for i in range(count):
            # Use the pipeline's camera object directly
            frame = pipeline.camera.read()
            if frame is None: 
                print(f"\n  WARNING: Frame {i+1} failed", file=sys.stderr)
                continue
            ...
            frames_captured += 1
            print(f"  Progress: {frames_captured}/{count}", end='\r')
        
        if accumulator is not None:
            # Calculate mean and convert to 16-bit PNG
            avg = (accumulator / frames_captured).clip(0, 65535).astype(np.uint16)
            cv2.imwrite(out_name, avg, [int(cv2.IMWRITE_PNG_COMPRESSION), 3])
            print(f"\nSUCCESS: Saved {desc} frame to {out_name}")
        else:
            print(f"\nERROR: Failed to acquire any frames for {desc}.")

    # --- Calibration Logic ---
    if args.dark_frame or args.bias_frame or args.flat_frame:
        ...
        if args.bias_frame:
            ...
            # Bias needs minimum possible exposure and gain
            config["capture"]["exposure_us"] = 100 
            config["capture"]["gain"] = 1.0
            desc, out_name = "Bias", "master_bias.png"
            
        elif args.dark_frame:
            print("--- Master Dark Creation ---")
            print("Cover the lens completely (Pitch black).")
            input("Press Enter to start...")
            # Darks use the current production settings for exposure/gain
            desc, out_name = "Dark", "master_dark.png"
            
        elif args.flat_frame:
            print("--- Master Flat Creation ---")
            print("Point camera at an evenly illuminated surface (white wall/t-shirt).")
            input("Press Enter to start...")
            desc, out_name = "Flat", "master_flat.png"

        app = Pipeline(cfg=config, simulate=args.simulate)
        try:
            capture_calib_stack(app, 50, desc, out_name)
        finally:
            if app.camera:
                app.camera.release()
        sys.exit(0)
